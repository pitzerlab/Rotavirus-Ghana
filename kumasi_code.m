...
load('kumasi_demographic_parameters.mat');
load('kumasi_model_parameters.mat');
...
% - -  for results  - - output names- -  - 
 cases_U5Y=zeros(100,120);
 cases_U1Y=zeros(100,120);
 cases_1Y=zeros(100,120);
 cases_2_4Y=zeros(100,120);
 age_dist=zeros(100,3);
...
%% - -  simulation length - - - - - -
t0=round(12*49);                            %burn-in period
tvacc=39;                                   %previccination period
...
datesim=(datenum([1960 1 1 0 0 0]):30.45:datenum([2030 12 31 0 0 0]))';  


 %------------ demographic parameters ------------------ ---------------------
N1=100000;      %population 
N=(N1/exp(.04*30))*agep; 
...
cdr=log(1+cdr)/12;
immig=.2*ones(length(cdr),1); 
immig=log(1+immig)/12;
um=.020*ones(1,al);
um=log(1+um-immig-.023)/12;  
...
pars=[33.661;0.7857;0.167;4.127;0.498;1.055;0.017;3.09e-07;0.649;5.33]; %mean
...
A_postvacc=A(40:159,1)'*sum(H(40:159,1:au5),2)/sum(sum(H(40:159,1:au5)));
agedist_prevacc=sum(Hu(1:39,1:au5)./sum(sum(Hu(1:39,1:au5),2)));
agedist_postvacc=sum(H(40:159,1:au5))./sum(sum(H(40:159,1:au5),2));
agedistV=sum(Hu(40:159,1:au5))./sum(sum(Hu(40:159,1:au5),2));
agedistU=sum(Hu(1:tvacc,1:au5))./sum(sum(Hu(1:tvacc,1:au5),2));



% results -- moderate-to-severe  cases 
H=[sum(H(1:264,[1:7]),2) sum(H(1:264,[8:12]),2) sum(H(1:264,[13:24]),2) sum(H(1:264,[25:27]),2)];
Hn=sum(H,2); 
cases_U5Y(icount,1:120)= Hn(40:159);                           %cases u5 years
cases_U1Y(icount,1:120)=H(40:159,1)+H(40:159,2);           %cases u1 year
cases_1Y(icount,1:120)=H(40:159,3);                        %cases 1Y  
cases_2_4Y(icount,1:120)=H(40:159,4);                      %cases 2-4Y 
age_dist(icount,1:3)=[sum(agedist_postvacc(1:12))  sum(agedist_postvacc(13:24)) sum(agedist_postvacc(25:27))] ;

%%- -  output file
save('kumasi_results.mat','cases_U5Y', 'cases_U1Y','cases_1Y', 'cases_2_4Y', 'age_dist');
